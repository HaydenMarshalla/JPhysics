...
public class Raycast {
    ...
    public Raycast(Vectors2D startPoint, Vectors2D direction, int distance) {
        this.startPoint = startPoint;
        this.direction = direction.getNormalized();
        this.distance = distance;
    }

    public Raycast(double direction, int distance) {
        this(new Vectors2D(), new Vectors2D(direction), distance);
    }

    public Raycast(Vectors2D direction, int distance) {
        this(new Vectors2D(), direction, distance);
    }

    public Raycast(Vectors2D startPoint, double direction, int distance) {
    ...
    public void draw(Graphics2D g, ColourSettings paintSettings, Camera camera) {
        ...
        if (intersectingBodiesInfo != null) {
            Vectors2D intersection = camera.scaleToScreen(intersectingBodiesInfo.coord);
...
//Contains information such as the intersection coordinate in world space and the body found.
class RayInformation {
    public final Body b;
    public final Vectors2D coord;
