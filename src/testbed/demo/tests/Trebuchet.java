...
public class Trebuchet {
    ...
    public static void load(TestBedWindow testBedWindow) {
        ...
        Joint j1 = new JointToPoint(new Vectors2D(20.469, 20.469), arm, 0, 100000, 1000, true, new Vectors2D(28.947, 0));
        temp.addJoint(j1);

        Body counterWeight = new Body(new Polygon(5.0, 10.0), 35.355, 11);
        counterWeight.setDensity(240);
        temp.addBody(counterWeight);

        Joint j2 = new JointToBody(arm, counterWeight, 20, 10000, 100, false, new Vectors2D(50, 0), new Vectors2D(0, 0));
        temp.addJoint(j2);

        Body payload = new Body(new Circle(5.0 ), 43.592, -35);
        payload.dynamicFriction = 0;
        payload.staticFriction = 0;
        payload.setDensity(50);
        temp.addBody(payload);

        Joint j3 = new JointToBody(arm, payload, 79, 10000, 100, true, new Vectors2D(-50, 0), new Vectors2D());
        temp.addJoint(j3);
    }
}